...
```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\backup_project.ps1 -ProjectRoot "." -BackupRoot ".\backups"
powershell -ExecutionPolicy Bypass -File .\scripts\backup_project.ps1 -Zip
powershell -ExecutionPolicy Bypass -File .\scripts\backup_project.ps1 -Zip -KeepFolder
powershell -ExecutionPolicy Bypass -File .\scripts\backup_project.ps1 -WhatIf
powershell -ExecutionPolicy Bypass -File .\scripts\backup_project.ps1 -Zip -KeepFolder -WhatIf
```

Beklenen çıktı:
- klasör modu: `Backup created: .\backups\project_backup_YYYYMMDD_HHMMSS`
- zip modu: `Backup created (zip): .\backups\project_backup_YYYYMMDD_HHMMSS.zip`
- dry-run modu: `[WHATIF] ...` adım logları + `Backup created (whatif): ...`

### 12.2 Restore (Geri Yükleme)
1. Streamlit/bot süreçlerini durdurun.
2. Geri dönmek istediğiniz kaynak backup’ı seçin (klasör veya zip):
   - `.\backups\project_backup_YYYYMMDD_HHMMSS`
   - `.\backups\project_backup_YYYYMMDD_HHMMSS.zip`
3. Tek komut restore scriptini çalıştırın:
```powershell
cd telegram_bot
powershell -ExecutionPolicy Bypass -File .\scripts\restore_project.ps1 -Source ".\backups\project_backup_YYYYMMDD_HHMMSS" -ProjectRoot "." -NoConfirm
```
Zip kaynak için:
```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\restore_project.ps1 -Source ".\backups\project_backup_YYYYMMDD_HHMMSS.zip" -ProjectRoot "." -NoConfirm
...
Dry-run restore için:
```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\restore_project.ps1 -Source ".\backups\project_backup_YYYYMMDD_HHMMSS.zip" -ProjectRoot "." -NoConfirm -WhatIf
```
...
## 14) Otomatik Preflight Kontrol
Aşağıdaki komut RUNBOOK güvenlik checklist’inin kritik teknik kontrollerini otomatik yapar:
```powershell
cd telegram_bot
powershell -ExecutionPolicy Bypass -File .\scripts\preflight_check.ps1
```
...
Beklenen:
- `.env` içinde en az şu anahtarlar bulunmalı:
  - `EXECUTION_ENABLED`
  - `PAPER_ONLY`
  - `USE_TESTNET`
  ...
  - `MAX_NOTIONAL_PCT`
  - `ORDER_SIZE_PCT`
- Güvenli varsayılan için: `EXECUTION_ENABLED=0`
- `DEFAULT_RISK_PROFILE` yalnızca: `low|balanced|aggressive`
- `ORDER_SIZE_PCT` numeric ve `(0,1]` aralığında olmalı
- `MAX_NOTIONAL_PCT` numeric ve `(0,1]` aralığında olmalı
...
- [x] Dashboard smoke test tamam
- [x] Dashboard refresh ON/OFF + manuel + kontrol değişimi test edildi
- [x] Dashboard performans (cache/hash) davranışı doğrulandı
- [x] Trade history filtre + CSV export senaryoları doğrulandı
- [x] Risk paneli metrik/progress/status doğrulandı
- [x] Pytest geçti
- [x] Launcher smoke test tamam (`start_dashboard.bat`)
- [x] Launcher log yazımı doğrulandı (`logs/launcher.log`)
- [x] Backup script çalıştı (`scripts/backup_project.ps1`)
- [x] Restore adımı dry-run tamamlandı
- [x] Canlı trade öncesi güvenlik checklist’i tamamlandı
- [x] Geri alma planı hazır (`ROLLBACK.md`)
